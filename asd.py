#!/usr/bin/env python3
"""
grabber_api_pdf.py
Fetch recent PDF samples from MalwareBazaar API (safe & direct, no CLI dependency).
Skips duplicates, saves each ZIP to ~/mb_downloads, logs seen hashes.

Usage:
  export MB_AUTH_KEY="your_api_key_here"
  python3 grabber_api_pdf.py
"""

import os, sys, time, json, pathlib, requests
from datetime import datetime

API_URL = "https://mb-api.abuse.ch/api/v1/"
STATE = pathlib.Path("~/.mb_seen_pdfs.txt").expanduser()
OUT_DIR = pathlib.Path("~/mb_downloads").expanduser()
LIMIT = int(os.getenv("MB_LIMIT", "200"))  # how many recent samples to consider

OUT_DIR.mkdir(parents=True, exist_ok=True)

def load_seen():
    if not STATE.exists():
        return set()
    return {line.strip() for line in STATE.read_text().splitlines() if line.strip()}

def append_seen(sha):
    STATE.parent.mkdir(parents=True, exist_ok=True)
    with STATE.open("a") as f:
        f.write(sha + "\n")

def api_post(payload, need_key=False, stream=False):
    key = os.getenv("MB_AUTH_KEY", "")
    headers = {
        "User-Agent": "mb-grabber/1.0 (+kali)",
        "API-KEY": key if key else "",
        "X-API-KEY": key if key else "",
        "Authorization": f"Bearer {key}" if key else "",
    }
    data = dict(payload)
    if need_key:
        if not key:
            raise RuntimeError("MB_AUTH_KEY not set; required for this request")
        data["api_key"] = key

    r = requests.post(API_URL, data=data, headers=headers, timeout=60, stream=stream)
    r.raise_for_status()
    return r

def fetch_recent_pdfs(limit=200):
    """Mimics 'bazaar recent', filters only PDFs."""
    payload = {"query": "get_recent", "selector": "time", "limit": str(limit)}
    r = api_post(payload, need_key=True)
    data = r.json()
    items = data.get("data", []) or []
    items = [it for it in items if (it.get("file_type") or it.get("filetype")) == "pdf"]

    def ts(x):
        s = (x.get("first_seen") or x.get("last_seen") or "").strip()
        try:
            return datetime.strptime(s, "%Y-%m-%d %H:%M:%S").timestamp()
        except Exception:
            return 0.0

    items.sort(key=ts, reverse=True)
    return items

def download_zip_for_sha(sha):
    """Download the sample ZIP file for a given SHA256 hash."""
    payload = {"query": "get_file", "sha256_hash": sha}
    r = api_post(payload, need_key=True, stream=True)
    ctype = r.headers.get("Content-Type", "")
    if "application/zip" not in ctype and "application/octet-stream" not in ctype:
        txt = r.text[:500]
        raise RuntimeError(f"Unexpected content-type ({ctype}). Server said: {txt}")

    fname = None
    cd = r.headers.get("Content-Disposition", "")
    if "filename=" in cd:
        fname = cd.split("filename=", 1)[1].strip().strip('"')
    if not fname:
        fname = f"{sha}.zip"

    out_path = OUT_DIR / fname
    with out_path.open("wb") as f:
        for chunk in r.iter_content(chunk_size=65536):
            if chunk:
                f.write(chunk)
    return out_path

def main():
    print(f"State file: {STATE}")
    print(f"Output dir: {OUT_DIR}")
    seen = load_seen()
    print(f"Loaded {len(seen)} seen SHAs.")

    print("Fetching recent PDF entries...")
    items = fetch_recent_pdfs(LIMIT)
    print(f"API returned {len(items)} total items.")

    new = []
    for it in items:
        sha = it.get("sha256_hash") or it.get("sha256")
        if not sha:
            continue
        if sha in seen:
            continue
        new.append(sha)

    if not new:
        print("No new PDF samples found.")
        return

    print(f"Will download {len(new)} new samples (newest first).")
    for i, sha in enumerate(new, 1):
        try:
            print(f"[{i}/{len(new)}] Downloading {sha} ...")
            path = download_zip_for_sha(sha)
            print(f"   Saved to {path}")
            append_seen(sha)
            time.sleep(float(os.getenv("MB_SLEEP", "0.5")))
        except Exception as e:
            print(f"   ERROR for {sha}: {e}", file=sys.stderr)

    print("Done. Latest files:")
    for p in sorted(OUT_DIR.glob("*.zip"), key=lambda x: x.stat().st_mtime, reverse=True)[:10]:
        print(" -", p)

if __name__ == "__main__":
    main()
